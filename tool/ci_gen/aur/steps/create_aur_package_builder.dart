...
class CreateAurPackageBuilder implements StepBuilder {
  ...
  Iterable<Step> build() => [
    const Step.run(
      ...
      run: 'generate-pkgbuild --input src --output aur > aur/.files',
    ),
    const Step.run(
      name: 'Change ownership of AUR repo to the build user',
      run: 'chown -R build:build aur',
    ),
    _suRun(name: 'Update source checksums', run: 'updpkgsums'),
    _suRun(name: 'Run namcap linter', run: 'namcap -i PKGBUILD'),
    _suRun(
      name: 'Test if package can be build',
      run: 'makepkg -sfC --check --noconfirm',
    ),
    _suRun(name: 'Generate SRCINFO', run: 'makepkg --printsrcinfo > .SRCINFO'),
    const Step.run(
  ...
  Step _suRun({required String name, required String run}) =>
      Step.run(name: name, run: "su build -c '$run'", workingDirectory: 'aur');
}
